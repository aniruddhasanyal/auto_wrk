...
def get_topic_dist(text=None, topics=10, top_words=20):
    ...
    clean_text = []
    for note in text:
        clean_text.append(' '.join([word for word in re.sub(r'[^\sa-zA-Z0-9]', '', str(note)).lower().split() if word not in stops]))
    ...
    clf = decomposition.NMF(n_components=topics, random_state=1, max_iter=500)
    W_an = clf.fit_transform(features)
    H_an = clf.components_

    for topic_index in range(H_an.shape[0]):
        topic_index = np.argsort(H_an[topic_index, :])[::-1][0:top_words]
        term_ranking = [features_names[i] for i in topic_index]
        print("Topic_an %d: %s" % (topic_index, ", ".join(term_ranking)))

    # Normalizing scores
    W_scaled = W_an / np.sum(W_an, axis=1, keepdims=True)

    claim_numbers = join_claim_adj_df_fin['ClaimNumber_x']

    # turn this into an array so we can use NumPy functions
    claim_numbers = np.asarray(claim_numbers)

    num_groups = len(set(claim_numbers))

    W_grouped = np.zeros((num_groups, topics))

    for i, num in enumerate(sorted(set(claim_numbers))):
        W_grouped[i, :] = np.mean(W_scaled[claim_numbers == num, :], axis=0)

    W_scaled_df = pd.DataFrame(list(W_grouped))

    # convert to list
    sorted_claim_numbers = sorted(set(claim_numbers))

    # convert lists to dataframes
    sorted_claim_numbers_df_an = pd.DataFrame({'Claim_Number': sorted_claim_numbers})

    # combine to one dataframe
    claim_topic_dist = pd.concat([sorted_claim_numbers_df_an, W_scaled_df], axis=1)

    # impute NaN values with zeroes
    claim_topic_dist.fillna(0, inplace=True)
    return claim_topic_dist


# Prepare structured data by executing SQL queries
exec_query()

# Pull loss desription from db
...
if 'Unnamed: 0' in unstr_claim_adj_df_full.columns:
    del unstr_claim_adj_df_full['Unnamed: 0']

if 'Unnamed: 0' in str_claim_adj_df.columns:
    del str_claim_adj_df['Unnamed: 0']

if 'DT_seg' in str_claim_adj_df.columns:
    del str_claim_adj_df['DT_seg']

# Obtain new complete(struct + unstruct) data by combining structured and text
...
# new Adjuster Notes appended to the old Adjuster Notes
join_claim_adj_df_fin = join_claim_adj_df.append(join_claim_adj_df_new)


# Handle matches and mismatches from previous data
...
df_unstr_LD = get_topic_dist(join_claim_adj_df_fin['Description'])
df_unstr_AN = get_topic_dist(join_claim_adj_df_fin['Adjuster_Notes'])
...
str_claim_adj_df['cd_cn'] = str_claim_adj_df['ClaimNumber']
Final_Data['cd_cn'] = '9999999' + Final_Data['ClaimNumber'].astype(str)
df_str = pd.concat([str_claim_adj_df, Final_Data], axis=0)

unstr_claim_adj_df_full.columns = ['claimID', 'ClaimNumber', 'Description', 'Adjuster_Notes']
new_unstr['cd_cn'] = '9999999' + new_unstr['ClaimNumber'].astype(str)
unstr_claim_adj_df_full['cd_cn'] = unstr_claim_adj_df_full['ClaimNumber']
df_unstr_orig = pd.concat([unstr_claim_adj_df_full, new_unstr], axis=0)
